                ...
                {loading ? (
                    ...
                    <div className="grid gap-4">
                        {appointments.filter(apt => apt.status !== 'pending' || (new Date().getTime() - new Date(apt.scheduled_start).getTime()) / 60000 < 240).map((apt, index) => (
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} key={apt.id} onClick={() => { setSelectedAptDetails(apt); setIsDetailsOpen(true); }} className={`relative glass-card group overflow-hidden transition-all duration-300 hover:border-white/10 hover:translate-x-1 cursor-pointer active:scale-[0.99] ${apt.status === 'active' ? 'border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.3)] bg-emerald-500/5' : ''} ${apt.requires_attention ? 'border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)] bg-amber-500/5' : ''}`}>
                                ...
                                <div className="p-6 flex flex-col md:flex-row md:items-center gap-6 md:gap-8">
                                    <div className="flex flex-col items-center gap-4 min-w-[120px]">
                                        ...
                                        <div className="hidden md:flex flex-col gap-1.5 w-full items-center">
                                            <div className="text-[9px] font-black text-slate-500 bg-white/5 px-2 py-1 rounded-lg border border-white/10 uppercase tracking-[0.1em] text-center w-full truncate max-w-[110px]">
                                    ...
                                    <div className="hidden md:block flex-grow space-y-4">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center flex-wrap gap-2"><div className="flex items-center gap-2 text-[11px] font-bold text-primary uppercase tracking-wider bg-primary/5 border border-primary/10 px-3 py-1.5 rounded-xl w-fit"><Sparkles size={12} /> {apt.treatment_name || 'Standard Session'}</div>{apt.required_skills?.map((skill, si) => <span key={si} className="text-[9px] font-black text-slate-500 bg-white/5 px-2 py-1 rounded-lg border border-white/10 uppercase tracking-[0.1em]">{skill}</span>)}</div>
                                                <div className="flex items-center gap-4 text-xs text-slate-400 font-medium ml-1"><span className="flex items-center gap-2 opacity-60 text-[10px] uppercase font-black tracking-widest">Duration: <span className="text-white">{apt.duration_minutes} min</span></span><div className="h-1 w-1 bg-white/10 rounded-full" /><span className="flex items-center gap-2 text-primary font-bold"><ArrowRight size={14} className="opacity-40" />Ends {format(new Date((apt.status === 'active' ? new Date(apt.actual_start).getTime() : new Date(apt.scheduled_start).getTime() + (apt.delay_minutes || 0) * 60000) + apt.duration_minutes * 60000), 'HH:mm')}</span></div>
                                            </div>
                                            {apt.notes && <p className="text-xs text-slate-500 italic line-clamp-1 border-l-2 border-primary/30 pl-3 py-0.5 bg-white/[0.01]">{apt.notes}</p>}
                                        </div>
                                    </div>
